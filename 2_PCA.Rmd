...
P.grassland <- rtruncnorm(a=0.10, b=.50, n=1000, mean = .40, sd = .10) %>% round(2)
P.open <- rtruncnorm(a=0.10, b = 0.40, n = 1000, mean = 0.30, sd = 0.10) %>% round(2)
P.closed <- rtruncnorm(a=.10, b=.30, n=1000, mean = .20, sd = .10) %>% round(2)
P.dense <- rtruncnorm(a=0.10, b = 0.40, n = 1000, mean = 0.10, sd = 0.10) %>% round(2)
...
SHEI <- rtruncnorm(a=0, b = 1, n = 1000, mean = 0.5, sd = 0.15)
pladj <- rtruncnorm(a=0, b = 1, n = 1000, mean = 0.5, sd = 0.15)
LSI <- rtruncnorm(a=1, b = 10, n = 1000, mean = 5, sd = 3)
```
...
size.change <- (sample(c(20:300), size = 1000, replace = TRUE))
...
SHEI.change <- (sample(c(20:50), size = 1000, replace = TRUE))*0.01
structure.M$SHEI <- structure.H$SHEI + SHEI.change

pladj.change <- (sample(c(20:45), size = 1000, replace = TRUE))*0.01
structure.M$pladj <- structure.H$pladj + pladj.change

LSI.change <-(sample(c(0:7), size = 1000, replace = TRUE))
...
levels(U$descriptors) <- c("Contagion", "Forest edge", 
                           ...
                           "Number of patches",
                           "Proportion closed forest",
                           "Percent canopy cover",
                           "Proportion dense forest",
                           "Proportion grassland",
                           "Proportion open forest",
                           "Mean patch size",
...
```{r}
ggsave('figures/2_PCA_biplot.jpeg',biplot1, units = 'cm', width = 15, height = 11)
```
